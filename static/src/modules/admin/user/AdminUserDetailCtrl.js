...
module.exports = function($scope, $state, $stateParams, $translate, toastr,
  ...
  $scope.loadData = function() {
    // get user
    User.get({id: userId}).$promise.then(function(res) { $scope.user = res; });

  };

  $scope.edit = function() {
    User.edit($scope.user).$promise.then(function(res) {
      $scope.user = res;
      toastr.success($translate.instant('SAVE_SUCCESS'));
    }).catch (function(err) { toastr.error(err.msg); });
  };

  $scope.loadUserProjectsDone = false;
  ...
  $scope.deleteUser = function(event) {
    ...
    $mdDialog.show(confirm).then(function() {
      User.delete ({id: $scope.user.id}).$promise.then(function() {
